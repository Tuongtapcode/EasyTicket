...
# ---- Helper: tạo fake query ----
def make_fake_query(obj):
    class FakeQuery:
        def get(self, _id):
            return obj
...
def test_createMomoPayment_success(order_mock, payment_mock):
    ...
    with patch("app.models.Order.query", make_fake_query(order_mock)), \
...
def test_createMomoPayment_order_not_found():
    ...
    with patch("app.models.Order.query", make_fake_query(None)):
...
def test_processIPN_invalid_signature(order_mock, payment_mock):
    ...
    with patch("app.models.Order.query", make_fake_query(order_mock)), \
            patch("app.models.Payment.query", make_fake_query(payment_mock)), \
            patch("app.services.momo_service.hmac_sha256_hex", lambda raw, key: "sig"):
